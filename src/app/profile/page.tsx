...
function ProfileComponent() {
  ...
  useEffect(() => {
    ...
    if (view && ['my-photos', 'liked-photos', 'dashboard'].includes(view)) {
      ...
      setActiveView('dashboard'); // Default to dashboard if view is invalid or not present
    }
  }, [searchParams]);

  const handleSignOut = () => {
    localStorage.removeItem('isLoggedIn');
    // We need to dispatch a storage event to trigger the useAuth hook in the header
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      const newImage: GalleryImage = {
        ...
        name: file.name.split('.').slice(0, -1).join('.'), // Use file name as title
